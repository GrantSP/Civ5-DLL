...
/// AMS: Edit the code in order to not attack a city with 1 HP with ranged.
...
/// AMS: Reworked to not intercept more than needed. This is done expecting the AI will call this both on Homeland and Tactical processes.
...
/// AMS: Rearranged to city atk -> melee move -> ranged move -> ranged atk -> melee atk.
void CvTacticalAI::ExecuteAttack(CvTacticalTarget* pTarget, CvPlot* pTargetPlot, bool bInflictWhatWeTake, bool bMustSurviveAttack)
{
	...
	// Next, lets apply damage from city bombards
	...
	// Third loop are only melee units that may attack
	for(unsigned int iI = 0; iI < m_CurrentMoveUnits.size() && iDamageRemaining > 0; iI++)
	{
		if(!bInflictWhatWeTake || m_CurrentMoveUnits[iI].GetExpectedTargetDamage() >= m_CurrentMoveUnits[iI].GetExpectedSelfDamage())
		{
...
/// AMS: Try to avoid water tiles if going to embark
		...
		if(pUnit)
		{
			...
			for(int iX = -iRange; iX <= iRange; iX++)
			{
				for(int iY = -iRange; iY <= iRange; iY++)
				{
					...
					//   AMS: Higher danger on water plots if is going to embark
...
//AMS: Iterate through available plots and get the best one to later move at.
...
//AMS: helper function to iterate vector that is of CvPlot Type.
...
/// AMS: Add out of range but in range+move units as half damage for calculations
int CvTacticalAI::ComputeTotalExpectedDamage(CvTacticalTarget* pTarget, CvPlot* pTargetPlot)
{
	...
	for(unsigned int iI = 0; iI < m_CurrentMoveUnits.size(); iI++)
	{
		...
		farAwayUnitDivisor = (plotDistance(pAttacker->getX(), pAttacker->getY(), pTargetPlot->getX(), pTargetPlot->getY()) > pAttacker-> GetRange()) ? 2 : 1;
